...
namespace DirectoryAnalyzer.Views
{
 ...
 main
                ...
                {
                    var data = moduleResult.Items[0].Columns;
                    var resumo = ParseJsonList(data.TryGetValue("ResumoJson", out var resumoJson) ? resumoJson : null);
                    var links = ParseJsonList(data.TryGetValue("LinksJson", out var linksJson) ? linksJson : null);
                    var delegacoes = ParseJsonList(data.TryGetValue("DelegacoesJson", out var delegacoesJson) ? delegacoesJson : null);
                    var securityFiltering = ParseJsonList(data.TryGetValue("SecurityFilteringJson", out var securityJson) ? securityJson : null);
                    var wmiFilters = ParseJsonList(data.TryGetValue("WmiFiltersJson", out var wmiJson) ? wmiJson : null);

                    ResumoGrid.ItemsSource = resumo;
                    LinksGrid.ItemsSource = links;
                    DelegacaoGrid.ItemsSource = delegacoes;
                    SecurityFilteringGrid.ItemsSource = securityFiltering;
                    WmiFiltersGrid.ItemsSource = wmiFilters;
        ...
        private IReadOnlyList<object> ParseJsonList(string json)
        {
            ...
            {
                list.Add(ParseJsonElement(document.RootElement));
            }
