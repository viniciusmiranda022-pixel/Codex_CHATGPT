...
namespace DirectoryAnalyzer.Agent
{
    ...
    public sealed class AgentConfig
    {
        ...
        [DataMember(Order = 18)]
        public string BrokerUrl { get; set; } = "https://localhost:5001/agent-hub";

        [DataMember(Order = 19)]
        public string BrokerClientCertificateThumbprint { get; set; } = string.Empty;
    ...
    public static class AgentConfigLoader
    {
        ...
        private static AgentConfig CreateFromRegistry(string path, out IReadOnlyList<string> overrides, out string logPathSource)
        {
            var config = new AgentConfig
            {
                ...
                BrokerUrl = ReadRegistryValue("BrokerUrl", "https://localhost:5001/agent-hub"),
                BrokerClientCertificateThumbprint = ReadRegistryValue("BrokerClientCertificateThumbprint", string.Empty),
                AgentId = ReadRegistryValue("AgentId", Environment.MachineName),
                Capabilities = ReadRegistryValue("Capabilities", string.Empty)
                    .Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries)
            };
        ...
        private static IReadOnlyList<string> ApplyRegistryOverrides(AgentConfig config, out string logPathSource)
        {
            ...
            var brokerUrl = ReadRegistryValue("BrokerUrl", null);
            if (!string.IsNullOrWhiteSpace(brokerUrl))
            {
                config.BrokerUrl = brokerUrl;
                overrides.Add("BrokerUrl");
            }

            var brokerClientCert = ReadRegistryValue("BrokerClientCertificateThumbprint", null);
            if (!string.IsNullOrWhiteSpace(brokerClientCert))
            {
                config.BrokerClientCertificateThumbprint = brokerClientCert;
                overrides.Add("BrokerClientCertificateThumbprint");
            }

            var agentId = ReadRegistryValue("AgentId", null);
            if (!string.IsNullOrWhiteSpace(agentId))
            {
                config.AgentId = agentId;
                overrides.Add("AgentId");
            }
            ...
            var capabilities = ReadRegistryValue("Capabilities", null);
            if (!string.IsNullOrWhiteSpace(capabilities))
            {
                config.Capabilities = capabilities.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
                overrides.Add("Capabilities");
            }
