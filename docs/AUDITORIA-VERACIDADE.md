...
26) **AgentConfig pode ser sobrescrita via registry.**
   * PROVADO — `ConfigLoader.ApplyRegistryOverrides`.
...
28) **O resolver do agente procura `agentsettings.json` em `%ProgramData%\DirectoryAnalyzer`.**
   * PROVADO — `DirectoryAnalyzer.Agent/Program.cs` e `AgentService/Program.cs`.

## Contradições encontradas
1) **Path de configuração do agente**
   * Instalação grava em `%ProgramData%\DirectoryAnalyzerAgent\agentsettings.json`, mas o host padrão lê `%ProgramData%\DirectoryAnalyzer\agentsettings.json`.
   * Plano de correção: alinhar o resolver do agente ao path do instalador **ou** alterar o instalador para escrever no path esperado pelo host.
